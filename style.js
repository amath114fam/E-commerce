...
const trash = document.getElementById("exit");
...
btnShop.addEventListener("click", () => {
  cartShop.classList.toggle("activee")
...
function addClicked(event) {
  ...
  showNotification(`Article ${title} a été ajouté au panier !`)
  updateTotal()
}
...
function achatValide() {
  const notification = document.createElement("div");
  notification.classList.add("notification");
  notification.textContent = 'Achat réussi!';
  document.body.appendChild(notification);
  setTimeout(() => {
    notification.remove();
  }, 3000);
